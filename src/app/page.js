...
export default function ClientComponent({ initialData }) {
  const [arenaGamesData, setArenaGamesData] = useState(initialData);

  useEffect(() => {
    console.log("Client-side logic executed in useEffect");
    // Ajoute ici ta logique spécifique au client si nécessaire
    setArenaGamesData(initialData);
  }, []);

  return (
    <div>
      {arenaGamesData && arenaGamesData.length > 0 ? (
        <div className="p-5 grid lg:grid-cols-5 md:grid-cols-3 sm:grid-cols-2">
          {arenaGamesData.map((game) => (
            <div key={game.geekId} className="p-4 mb-4">
              <Tilt
